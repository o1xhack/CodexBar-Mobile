...
final class StatusMenuTokenAccountSwitcherTests: XCTestCase {
    ...
    func test_tokenAccountMenuSelectionRefreshesProviderWhileGlobalRefreshIsActive() async throws {
        ...
        XCTAssertGreaterThanOrEqual(startedBeforeDrain, 1)

        await blocker.resumeAll(with: .success(self.snapshot(percent: 17)))
        if startedBeforeDrain < 2 {
    ...
    func test_segmentedRefreshPreservesValidAccountCacheAndInvalidatesCredentialChanges() async {
        ...
        settings.refreshFrequency = .manual
        settings.multiAccountMenuLayout = .segmented
        // This upstream cache test exercises Mac-only segmented fetching.
        // The fork intentionally fans out every account when iCloud sync is
        // enabled so iPhone receives all tabs; that contract has dedicated
        // coverage in ShouldFetchAllTokenAccountsTests.
