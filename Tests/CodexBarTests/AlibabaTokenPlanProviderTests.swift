...
// Keep this fixture-driven provider suite together so its regional and account-mode contracts stay reviewable.
...
/// Dedicated protocol stub for Personal/Solo retry tests. Those requests use a
/// production-shaped gateway host, and isolating their handler prevents the
/// file's legacy Team/rate-limit suites from replacing it when suites run in
/// parallel.
final class AlibabaTokenPlanPersonalStubURLProtocol: URLProtocol {
...
struct AlibabaTokenPlanPersonalUsageRetryTests {
    ...
    private static func stubSession() -> URLSession {
        let configuration = URLSessionConfiguration.ephemeral
        configuration.protocolClasses = [AlibabaTokenPlanPersonalStubURLProtocol.self]
        return URLSession(configuration: configuration)
    }

    @Test
    func `recovers when an empty Success usage response is followed by a full one`() async throws {
        defer { AlibabaTokenPlanPersonalStubURLProtocol.handler = nil }
        ...
        AlibabaTokenPlanPersonalStubURLProtocol.handler = Self.personalHandler(
