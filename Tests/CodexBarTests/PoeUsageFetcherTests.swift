...
struct PoeUsageFetcherTests {
    ...
    @Test
    func `snapshot maps balance to identity loginMethod and generic RateWindow`() {
        let snapshot = PoeUsageSnapshot(
            currentPointBalance: 500,
            updatedAt: Date())

        let unified = snapshot.toUsageSnapshot()
        #expect(unified.primary?.usedPercent == 0)
        #expect(unified.primary?.resetDescription == "Balance: 500 points")
    ...
    @Test
    func `snapshot maps points history to generic windows for iOS`() {
        let now = Date(timeIntervalSince1970: 1_800_000_000)
        let history = PoeUsageHistorySnapshot(
            entries: [],
            daily: [
                .init(day: "2026-05-29", points: 100, requests: 1, costUSD: nil),
        ...
        #expect(unified.primary?.resetDescription == "Balance: 500 points")
        #expect(unified.secondary?.resetDescription == "30d: 600 points · 6 requests")
        #expect(unified.extraRateWindows?.map(\.title) == ["Today", "7d"])
        #expect(unified.extraRateWindows?.map(\.window.resetDescription) == [
            "Today: 300 points · 3 requests",
            "7d: 600 points · 6 requests",
        ])
        #expect(unified.poeUsage?.daily.count == 3)
    }
