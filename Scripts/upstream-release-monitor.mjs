...
async function main() {
  ...
  for (const release of newReleases) {
    ...
    if (reusableGenericIssue) {
      console.log(`${dryRun ? 'Would update' : 'Updating'} generic issue #${reusableGenericIssue.number} for ${release.tag_name}`);
      if (!dryRun) {
...
function isTrackableRelease(release) {
  return !release.draft
    && !release.prerelease
    && Boolean(parseTag(release.tag_name));
}
...
function inferImpactRows(body) {
  ...
  addIf(
    rows,
    /Kiro:/i.test(body),
    'Kiro CLI discovery 修正',
    '❌ macOS CLI 环境检测专属，iOS 通常不适用',
    '-',
  );
  addIf(
    rows,
