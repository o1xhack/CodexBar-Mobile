...
| v0.54.0.1-mobile.1.21.0 | ❌ 不需要 | 最后published `v0.52.0.1-mobile.1.21.0` → candidate 的`CloudConstants.swift`无schema diff，`providerPayloadVersion=1`；provenance、coverage、token mix、metered cost与history coverage都是既有`DeviceProviderSnapshot.payload`内的optional JSON。2026-08-21 Production export仍为同一组10 types，无新type/field/index/zone/query/subscription。 |
