...
| v0.47.0.1-mobile.1.20.0 | ✅ **需要，待授权** | upstream fleet sync 新增 `CodexBarSync` zone 与 `AccountSnapshot`、`Device`、`Preferences`、`ProviderIntent`；2026-08-03 Production 只读 export 还确认缺少已有 iOS account-linkage writer 使用的 `ProviderAccountLinkage`。tracked schema 是当前 Production 5 types 与上述 5 个缺失 types 的完整 union，Development validation 已通过；未 import/deploy。Production 必须在单独授权后从 CloudKit Console promotion，并用 Production export readback。 |
