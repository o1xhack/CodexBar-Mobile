...
public struct Kimi2UsageFetcher: Sendable {
    ...
    public static func fetchCodeAPIUsage(
        apiKey: String,
        baseURL: URL = Kimi2SettingsReader.defaultCodeAPIBaseURL,
        identityHeaders: [String: String] = [:],
        now: Date = Date(),
        transport: any ProviderHTTPTransport = ProviderHTTPClient.shared) async throws -> Kimi2UsageSnapshot
    {
        ...
        guard response.statusCode == 200 else {
            ...
            Self.log.error("Kimi2 Code API returned \(response.statusCode): \(responseBody)")
    ...
    static func _parseCodeAPIUsageForTesting(_ data: Data, now: Date = Date()) throws -> Kimi2UsageSnapshot {
    ...
    static func _codeAPIErrorForTesting(statusCode: Int) -> Kimi2APIError {
        self.codeAPIError(statusCode: statusCode)
    }

    public static func fetchUsage(authToken: String, now: Date = Date()) async throws -> Kimi2UsageSnapshot {
    ...
    static func _fetchUsageForTesting(
        ...
        subscriptionGrace: Duration) async throws -> Kimi2UsageSnapshot
    {
        try await self.fetchUsage(
            authToken: authToken,
            now: now,
        ...
        subscriptionGrace: Duration) async throws -> Kimi2UsageSnapshot
    {
        ...
        let subscriptionTask = Task<Kimi2SubscriptionStatsResponse?, Error> {
        ...
        let subscriptionStats: Kimi2SubscriptionStatsResponse?
        switch subscriptionOutcome {
        case let .value(response):
            subscriptionStats = response
        case .timedOut:
            Self.log.warning("Kimi2 subscription stats timed out")
            subscriptionStats = nil
        case let .failure(error):
            Self.log.warning("Kimi2 subscription stats unavailable: \(error.localizedDescription)")
            subscriptionStats = nil
        }

        return Kimi2UsageSnapshot(
    ...
    private static func fetchRequiredUsage(
        authToken: String,
        sessionInfo: SessionInfo?,
        transport: any ProviderHTTPTransport) async throws -> Kimi2Usage
    {
        ...
        guard response.statusCode == 200 else {
            let responseBody = String(data: data, encoding: .utf8) ?? "<binary data>"
            Self.log.error("Kimi2 API returned \(response.statusCode): \(responseBody)")

            if response.statusCode == 401 {
                throw Kimi2APIError.invalidToken
            }
            if response.statusCode == 403 {
                throw Kimi2APIError.invalidToken
            }
            if response.statusCode == 400 {
                throw Kimi2APIError.invalidRequest("Bad request")
            }
            throw Kimi2APIError.apiError("HTTP \(response.statusCode)")
        }

        let usageResponse = try JSONDecoder().decode(Kimi2UsageResponse.self, from: data)
        guard let codingUsage = usageResponse.usages.first(where: { $0.scope == "FEATURE_CODING" }) else {
            throw Kimi2APIError.parseFailed("FEATURE_CODING scope not found in response")
        }

        return codingUsage
    }

    private static func parseCodeAPIUsage(from data: Data, now: Date) throws -> Kimi2UsageSnapshot {
        let response = try JSONDecoder().decode(Kimi2CodeAPIUsageResponse.self, from: data)
        return Kimi2UsageSnapshot(
    ...
    private static func fetchSubscriptionStats(
        authToken: String,
        sessionInfo: SessionInfo?,
        transport: any ProviderHTTPTransport) async throws -> Kimi2SubscriptionStatsResponse?
    {
        ...
        do {
            let response = try await transport.response(for: request)
            guard response.statusCode == 200 else {
                Self.log.warning("Kimi2 subscription stats returned \(response.statusCode)")
                return nil
            }
            return try JSONDecoder().decode(Kimi2SubscriptionStatsResponse.self, from: response.data)
        } catch {
    ...
    private static func codeAPIError(statusCode: Int) -> Kimi2APIError {
        switch statusCode {
        case 400:
