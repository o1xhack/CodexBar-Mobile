...
public enum Kimi2CookieImporter {
    ...
    private static let cookieImportOrder: BrowserCookieImportOrder =
        ProviderDefaults.metadata[.kimi2]?.browserCookieOrder ?? Browser.defaultImportOrder
        ...
        logger: ((String) -> Void)? = nil) throws -> [SessionInfo]
    {
        ...
        guard !sessions.isEmpty else {
            throw Kimi2CookieImportError.noCookies
        }
        return sessions
    }
...
enum Kimi2CookieImportError: LocalizedError {
    case noCookies

    var errorDescription: String? {
        switch self {
        case .noCookies:
            "No Kimi2 session cookies found in browsers."
        }
    }
}
#endif
