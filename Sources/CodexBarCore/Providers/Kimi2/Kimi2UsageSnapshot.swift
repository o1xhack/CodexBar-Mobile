...
public struct Kimi2UsageSnapshot: Sendable {
    public let weekly: Kimi2UsageDetail
    public let rateLimit: Kimi2UsageDetail?
    public let updatedAt: Date
    let subscriptionBalance: Kimi2SubscriptionBalance?
    let subscriptionCodeWeeklyLimit: Kimi2SubscriptionRateLimit?

    public init(weekly: Kimi2UsageDetail, rateLimit: Kimi2UsageDetail?, updatedAt: Date) {
        self.weekly = weekly
    ...
    init(
        weekly: Kimi2UsageDetail,
        rateLimit: Kimi2UsageDetail?,
        subscriptionBalance: Kimi2SubscriptionBalance?,
        subscriptionCodeWeeklyLimit: Kimi2SubscriptionRateLimit? = nil,
        updatedAt: Date)
    {
        self.weekly = weekly
        self.rateLimit = rateLimit
...
extension Kimi2UsageSnapshot {
    public func toUsageSnapshot() -> UsageSnapshot {
        ...
        let monthlyWindow = self.subscriptionBalance.flatMap { balance -> NamedRateWindow? in
            ...
            return NamedRateWindow(id: "kimi2-monthly", title: "Monthly", window: window)
        }
        ...
        let subscriptionCodeWeeklyWindow = self.subscriptionCodeWeeklyLimit.flatMap { limit -> NamedRateWindow? in
            ...
            let window = RateWindow(
                ...
                resetDescription: nil)
            return NamedRateWindow(id: "kimi2-code-7d", title: "Code 7-day", window: window)
        }
